...
**新規プロジェクト:**
```
/spec-mesh vision → clarify → design → clarify
→ /spec-mesh issue (Foundation) → plan → tasks → implement → pr
```

**機能追加:**
```
/spec-mesh add → clarify → plan → tasks → implement → pr
```

**バグ修正:**
```
/spec-mesh fix → clarify → plan → tasks → implement → pr
```
...
2. **推測禁止**: 不明点は `/spec-mesh clarify` で解消
3. **小さな変更**: レビューしやすい差分を心がける
4. **テスト必須**: 仕様に沿った挙動を保証
