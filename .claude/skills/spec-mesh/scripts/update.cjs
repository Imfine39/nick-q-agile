#!/usr/bin/env node
/**
 * SSD-MESH Template Update Script
 *
 * Updates the spec-mesh framework from the template repository while preserving
 * project-specific files.
 *
 * Usage:
 *   node .claude/skills/spec-mesh/scripts/update.cjs [options]
 *
 * Options:
...
// Configuration
const CONFIG = {
  templateRepo: 'Imfine39/ssd-template',
  branch: 'main',
...
function httpsGet(url) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, { headers: { 'User-Agent': 'ssd-mesh-updater' } }, (res) => {
...
async function downloadAndExtract(repoRoot, dryRun = false) {
  ...
  const extracted = fs.readdirSync(tempDir).find((f) => f.startsWith('ssd-template'));
  if (!extracted) {
...
function mergeCLAUDEmd(templateDir, repoRoot, dryRun = false) {
  ...
  const templatePath = path.join(templateDir, '.claude/skills/spec-mesh/templates/CLAUDE.template.md');
