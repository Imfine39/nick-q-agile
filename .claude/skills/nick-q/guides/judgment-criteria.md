...
### 「複合変更（Add 中に Change が必要になった等）の規模判定」

Feature 追加中に既存 Spec の変更が必要になった場合、以下の基準で分離判定を行う。

**規模別判定表:**

| 規模 | 条件 | 対応 | Issue |
|------|------|------|-------|
| **小規模** | 属性 1-3 個追加、単一 Spec 内、新規 ID 不要 | 同一 Issue で対応（Cascade Update） | 分離不要 |
...
| 状況 | 規模 | 対応 |
|------|------|------|
| M-CUSTOMER に `phone_number` 属性を1個追加 | 小規模 | 同一 Issue |
| M-ORDER に 5 個の属性を追加、Screen も変更 | 中規模 | 分離を提案 |
| 新規 M-PAYMENT を追加、API-PAYMENT-* を追加 | 大規模 | 必ず分離 |
...
Add/Fix 実行中に既存 Spec 変更が必要
    │
    ├─ 属性 1-3 個、1 Spec のみ → 小規模 → Cascade Update で対応
    │
