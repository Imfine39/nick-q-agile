...
Format for each task line (checkbox format for progress tracking):
...
- [x] [T-003] [P2] [UC-002] [S-LEADS-002] Implement UI for filtering customer list in frontend/src/pages/clients.tsx ← completed example

Where:

- `[ ]` = pending, `[x]` = completed (update when task is done)
...
**Progress Update Rule:** Mark `[x]` immediately after completing each task. Do not batch updates.
