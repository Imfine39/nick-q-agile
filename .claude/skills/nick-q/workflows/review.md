...
```
┌─────────────────────────────────────────────────────────────────┐
│ Before: 3 agents 並列 → 手動統合                                │
│   → 重複排除が煩雑、Severity 判定がブレる                       │
├─────────────────────────────────────────────────────────────────┤
│ After: 1 agent 3パス → 統合テーブル出力                         │
│   → 重複は自動マージ、Severity は基準に従い判定                 │
└─────────────────────────────────────────────────────────────────┘
```
...
### Before (3 agents 並列)
```
Step 2: 3 Task tool 呼び出し（並列）
Step 3: 手動で重複排除・マージ
```

### After (1 agent 3パス)
```
Step 2: 1 Task tool 呼び出し（統合テーブル出力）
Step 3 は統合済みなので不要 → AI 修正へ直行
```
