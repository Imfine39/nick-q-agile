...
```bash
node .claude/skills/nick-q/scripts/scaffold-spec.cjs --kind {kind} --id {id} --title "{title}"
```
...
```bash
node .claude/skills/nick-q/scripts/state.cjs repo --set-{kind}-status scaffold
```
