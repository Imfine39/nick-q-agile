...
```bash
node .claude/skills/nick-q/scripts/state.cjs branch --set-step pr
```
...
```bash
node .claude/skills/nick-q/scripts/post-merge.cjs --feature {FEATURE_ID} --delete-branch
```

**Example:**
```bash
node .claude/skills/nick-q/scripts/post-merge.cjs --feature S-AUTH-001 --delete-branch
```
