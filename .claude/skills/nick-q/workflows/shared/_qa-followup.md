...
**重要:** AskUserQuestion ツールは最大4問の制限があるが、質問全体がその制限に縛られるわけではない。

```
質問が 8 件ある場合:

Batch 1/2 (1-4/8)
├─ Q1-Q4 を AskUserQuestion で確認
├─ 回答を記録
└─ 自動で Batch 2 へ

Batch 2/2 (5-8/8)
├─ Q5-Q8 を AskUserQuestion で確認
├─ 回答を記録
└─ 完了
```

**自動継続ルール:**
1. 4問完了後、残りの質問があれば**ユーザー確認なし**で次のバッチを開始
2. 各バッチ開始時に進捗を表示: `=== 追加確認 (5-8 / 8) ===`
3. 全質問完了まで継続
