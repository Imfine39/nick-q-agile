 ...
 *
 * Usage:
 *   node .claude/skills/nick-q/scripts/branch.cjs --type feature --slug user-auth --issue 123
 *
 * Behavior:
...
function showHelp() {
  console.log(`
Branch creation helper for nick-q.

Usage:
  node .claude/skills/nick-q/scripts/branch.cjs --slug <name> [options]
...
Examples:
  node .claude/skills/nick-q/scripts/branch.cjs --type feature --slug user-auth --issue 123
  node .claude/skills/nick-q/scripts/branch.cjs --type fix --slug login-bug --issue 456
`);
}
