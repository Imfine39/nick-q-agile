...
- [ ] CHK-S-005: Non-functional requirements (performance, security, observability) are captured where applicable.
...
- [ ] CHK-T-005: Tasks exist for updating Overview spec and dependent Feature specs if shared masters/APIs change.
...
- [ ] CHK-G-005: `gh` (GitHub CLI) is available and used where automation is expected.
...
- [ ] CHK-TEST-003: No plan to weaken or remove tests solely to pass CI.
- [ ] CHK-TEST-004: There is an explicit plan to diagnose failures:

  - Spec bug vs test bug vs implementation bug vs environment issue.

- [ ] CHK-TEST-005: Changes to tests (if any) are documented with reasoning.
